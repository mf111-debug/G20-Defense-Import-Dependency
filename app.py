...
def load_arms_data(country_name):
    ...
    Handles both ANNUAL files (refYear=52) and MONTHLY files (refMonth=YYYYMM).
    """
    ...
    for fpath in sorted(files):
        ...
        # Annual: refYear column is all 52 (annual aggregate code)
        # Monthly: refMonth column has YYYYMM values like 201503
        if 'refYear' in df.columns and df['refYear'].dropna().isin([52]).all():
            # ANNUAL FILE
            # refMonth column contains the actual year (2021, 2022...)
            annual = df.groupby('refMonth')['fobvalue'].sum().reset_index()
            ...
            # MONTHLY FILE
            # refMonth column has YYYYMM format
...
with st.sidebar:
    ...
    available = [c for c, v in COUNTRIES.items() if v["region"] in selected_regions]

    st.markdown("### 🏳️ Select Countries")
    selected = st.multiselect(
        "Choose countries to compare",
        options=available,
        default=[c for c in ["Saudi Arabia", "South Korea", "Türkiye"] if c in available],
    ...
    show_trend  = st.checkbox("Show trend line", value=True,
                              help="Smooths month-to-month noise to show the overall direction")
...
st.caption("Percentage of military budget spent on importing weapons from abroad — lower = more self-sufficient")
...
for country, df in country_data.items():
    ...
    ax.plot(df['Date'], df['Dependency_Pct'],
            color=color, linewidth=0.7, alpha=0.2)

    if show_trend:
        trend = df['Dependency_Pct'].rolling(window=12, min_periods=3).mean()
        ax.plot(df['Date'], trend, color=color, linewidth=2.4,
    ...
    if show_peaks:
        idx      = df['Dependency_Pct'].idxmax()
        peak_val = df.loc[idx, 'Dependency_Pct']
        peak_dt  = df.loc[idx, 'Date']
        ax.plot(peak_dt, peak_val, 'o', color=color, markersize=7, zorder=5)
        ax.annotate(
            f"{flag} {peak_val:.1f}%",
            xy=(peak_dt, peak_val),
