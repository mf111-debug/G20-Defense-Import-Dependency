...
with st.sidebar:
    ...
    debug_mode  = st.checkbox("🔍 Show file debug info", value=False,
                              help="Shows which files loaded and what years were found")
    st.markdown("---")
    st.markdown("""
...
# Debug output
if debug_mode:
    with st.expander("🔍 File Loading Debug", expanded=True):
        for country, lines in all_debug.items():
