...
with st.sidebar:
    ...
    show_rolling = st.checkbox("12-Month Rolling Average", value=True)
    show_v2030 = st.checkbox("Vision 2030 Line (2016)", value=True)
    show_peak = st.checkbox("Yemen Conflict Annotation", value=True)
...
if show_v2030:
    ax.axvline(pd.Timestamp('2016-04-01'), color='#888888',
               linestyle='--', linewidth=1.2, alpha=0.6, label='Vision 2030 Launch')

if show_peak and 'Saudi Arabia' in country_data:
    sa = country_data['Saudi Arabia']
    idx = sa['Import_Dependency_Pct'].idxmax()
    ax.annotate('Peak: May 2015\nYemen Conflict',
                xy=(sa.loc[idx, 'Date'], sa.loc[idx, 'Import_Dependency_Pct']),
                xytext=(pd.Timestamp('2012-06-01'), sa.loc[idx, 'Import_Dependency_Pct'] * 0.85),
                arrowprops=dict(arrowstyle='->', color='#CCCCCC'),
                fontsize=8, color='#FFFF99',
