...
# ── Path fix: ensures files are found on Streamlit Cloud ─────────────────────
...
def load_sipri_csv():
    try:
        df = pd.read_csv(os.path.join(BASE_DIR, 'G20_Military_Expenditure_1949_2025.csv'))
...
def load_arms_data(country_name):
    ...
    files = sorted(glob.glob(os.path.join(BASE_DIR, f"{prefix}*.csv")))
    if not files:
